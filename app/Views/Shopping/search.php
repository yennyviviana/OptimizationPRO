...
if (isset($_GET['query'])) {
    ...
    $sql = "SELECT * FROM  compras WHERE   productos_comprados LIKE ? OR detalles_productos LIKE ?";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            echo "<p>" . htmlspecialchars($row['productos_comprados']) . " - $" . number_format($row['precio_unitario'], 2, ',', '.') . "</p>";
        }
    } else {
